    ...
    if start_excel:
        with st.spinner(
            ...
            try:
                ...
                st.error("❌ Không thể xử lý Excel.")
                st.exception(exc)
...
# ============================================================
    ...
    if start_image:
        with st.spinner("Đang OCR và dịch hình ảnh..."):
            try:
                ...
                st.error("❌ Không thể xử lý hình ảnh.")
                st.exception(exc)
else:
    st.error("Định dạng file chưa được hỗ trợ.")
