...
def looks_like_code_or_id(text):
    ...
    # Mã kiểu ABC-123 / PN12345
    if re.fullmatch(r"[A-Z]{2,10}[-_/]?\d{2,}", text):
        return True

    # Serial chỉ gồm chữ hoa + số
...
def extract_json(text):
    """
    Gemini đôi khi trả về markdown JSON.
    Hàm này cố lấy JSON ra an toàn.
    """
...
"""
    ...
    try:
        ...
        try:
            ...
            if len(result) < len(items):
                raise ValueError(
                    "Fallback Gemini trả thiếu bản dịch."
                )
...
def copy_row_dimension(ws, source_row, target_row):
    """
    Copy chiều cao và các thuộc tính row.
    """
...
def get_merged_range_for_row(ws, row):
    """
    Trả về merged ranges có liên quan đến row.
    """
...
def snapshot_workbook_structure(wb):
    """
    Lưu thông tin cấu trúc trước khi thay đổi.
    """
...
def duplicate_row_styles(ws, source_row, target_row, max_col):
    ...
    for col in range(
        ...
        # Hyperlink
        if src.hyperlink:
            dst._hyperlink = copy.copy(
                src.hyperlink
            )

        # Comment
        if src.comment:
            dst.comment = copy.copy(
                src.comment
            )
...
def build_translation_rows(
    ...
    Chiến lược:
    - xử lý từ dưới lên;
    - chỉ nhân đôi những hàng có text cần dịch;
    - hàng gốc giữ nguyên nếu Trung → Việt;
    - Việt → Trung: hàng trên là tiếng Trung,
      hàng dưới là tiếng Việt.
    """

    # Group theo sheet / row
    rows_by_sheet = {}
    ...
    for ws in wb.worksheets:
        ...
        for row in sorted(
            ...
            # Snapshot merged ranges liên quan
            merged_before = []
            ...
            # Copy toàn bộ style/value trước
            for col in range(
                1,
                original_max_col + 1,
            ):
            ...
            # ------------------------------------------------
            # Khôi phục merge theo từng dòng
            # ------------------------------------------------
            ...
            for (
                ...
                # Trường hợp merge nằm hoàn toàn trên
                # đúng một row
                if (
                    min_row == row
                    and max_row == row
                    ...
                    # xử lý bảo thủ.
                    #
                    # Không tự động gộp toàn bộ thành
                    # một merge khổng lồ.
                    #
                    # Khôi phục vùng đã dịch xuống.
                    shifted_min_row = (
                        min_row
                        + (
                            1
...
def apply_translations_to_excel(
    wb,
    translation_map,
    original_items,
):
    """
    Hàm chính xử lý Excel.
    """
...
def image_font(size, bold=False):
    """
    Tìm font Unicode có thể hiển thị Trung/Việt.
    """
...
def ocr_image_with_gemini(
    ...
    """
    Gemini OCR:
    trả về text + bounding box normalized 0-1000.
    """
...
def process_image(
    ...
    """
    OCR + dịch + vẽ tiếng Việt dưới tiếng Trung.
    ...
    for item in ocr_items:
        ...
        # Với Việt -> Trung:
        # ảnh gốc có Việt.
        # Ta vẫn giữ ảnh gốc nhưng thêm tiếng Trung
        # phía trên vùng Việt.

        if direction == "Trung → Việt":

            # Đặt tiếng Việt ngay dưới vùng OCR
            tx = x1
            ty = y2 + 4

            # Nếu vượt ảnh -> đặt vào khoảng phù hợp
