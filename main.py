    ...
    if extension in EXCEL_EXTENSIONS:
        ...
        if process_button:
            try:
                ...
                # Lưu kết quả vào session_state để nút download hiển thị bền vững
                st.session_state["excel_output_bytes"] = output.getvalue()
                st.session_state["excel_filename"] = f"dich_song_ngu_{Path(uploaded_file.name).stem}.{extension}"
                st.session_state["excel_mime"] = (
                    "application/vnd.ms-excel.sheet.macroEnabled.12"
                    if extension == "xlsm"
                    else "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
                st.session_state["excel_sheets_len"] = len(wb.sheetnames)
                st.session_state["excel_translated_count"] = translated_count
                st.session_state["excel_used_model"] = used_model

            except Exception as e:
                st.error("❌ Xử lý Excel thất bại.")
                st.exception(e)

        # Hiển thị nút download nếu đã có kết quả trong session_state
        if "excel_output_bytes" in st.session_state:
            st.download_button(
                label="📥 DOWNLOAD EXCEL SONG NGỮ",
                data=st.session_state["excel_output_bytes"],
                file_name=st.session_state["excel_filename"],
                mime=st.session_state["excel_mime"],
                use_container_width=True,
                type="primary",
            )
            ...
            with col1:
                st.metric("Sheets", st.session_state["excel_sheets_len"])
            with col2:
                st.metric("Ô đã dịch", st.session_state["excel_translated_count"])
            with col3:
                st.metric("Model", st.session_state["excel_used_model"])
